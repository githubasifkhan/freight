...
class FreightOperation(models.Model):
    ...
    def compute_total_amount(self):
        for order in self:
            invoices = self.env['account.move'].sudo().search([('freight_operation_id', '=', order.id), ('type', '=', 'out_invoice')])
            invoice_amount = 0.0
            bill_amount = 0.0
            ...
            bills = self.env['account.move'].sudo().search([('freight_operation_id', '=', order.id), ('type', '=', 'in_invoice')])
            for bill in bills:
    ...
    def _compute_invoice(self):
        for order in self:
            ...
            order.invoice_count = self.env['account.move'].sudo().search_count([('freight_operation_id','=', order.id),('type','=', 'out_invoice')])
            order.vendor_bill_count  = self.env['account.move'].sudo().search_count([('freight_operation_id','=', order.id),('type','=', 'in_invoice')])
